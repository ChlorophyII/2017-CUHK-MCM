...
%%trial code:
%Following is for single plot(total)
figure();
vec=dataMatrix(1:PER_PLOT_DATA_VOLUME,8)+dataMatrix(1:PER_PLOT_DATA_VOLUME,7);
plotMatrix=vec2mat(vec,length(AVG_CAR_PER_SEC));
x=SELF_RATIO;
y=AVG_CAR_PER_SEC;
[X,Y]=meshgrid(x,y);
...
handle=surf(X,Y,Z);
xlabel('SELF\_RATIO');
ylabel('AVG\_CAR\_PER\_SEC');
zlabel('queueSize');
title('........%i',1);
saveas(handle,'FIrstplot.jpg');



handle1=figure();
%Following template is for seperated plotting
vec=dataMatrix(PER_PLOT_DATA_VOLUME+1:2*PER_PLOT_DATA_VOLUME,8);
plotMatrix=vec2mat(vec,length(AVG_CAR_PER_SEC));
x=SELF_RATIO;
y=AVG_CAR_PER_SEC;
[X,Y]=meshgrid(x,y);
...
vec=dataMatrix(PER_PLOT_DATA_VOLUME+1:2*PER_PLOT_DATA_VOLUME,7);
plotMatrix=vec2mat(vec,length(AVG_CAR_PER_SEC));
x=SELF_RATIO;
y=AVG_CAR_PER_SEC;
[X,Y]=meshgrid(x,y);
